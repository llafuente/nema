...
class Angular5Client {
    static generate(api, dstPath, lint) {
        ...
        fs.copyFileSync(path.join(process.cwd(), "templates", "angular5client", "tsconfig.json"), path.join(dstPath, "tsconfig.json"));
        fs.copyFileSync(path.join(process.cwd(), "templates", "angular5client", "IsError.pipe.ts"), path.join(dstPath, "src", "IsError.pipe.ts"));
    ...
    static templates(dstPath) {
        ["Cast.ts", "CommonException.ts", "IsError.pipe.ts", "Random.ts"].forEach((filename) => {
            fs.copyFileSync(path.join(process.cwd(), "templates", "angular5client", filename), path.join(dstPath, filename));
        });
    }
