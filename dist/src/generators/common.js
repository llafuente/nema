...
function pretty(dstPath) {
    child_process_1.spawnSync(path.join(nemaRoot, "node_modules/.bin/prettier.cmd"), [
        "--write",
        "--parser",
        "typescript",
...
function lint(dstPath) {
    child_process_1.spawnSync(path.join(nemaRoot, "node_modules/.bin/tslint.cmd"), ["-c", "./tslint.json", "--project", JSON.stringify(path.join(dstPath + "/tsconfig.json")), "--fix"], {
        cwd: process.cwd(),
        env: process.env,
        shell: true,
        stdio: "inherit",
...
function modelInterface(api, model, ts) {
    ...
    _.each(model.properties, (t, name) => {
        s.push(`${name}: ${utils_1.schemaObjectTS(t)},`);
    });
    s.push(`}`);
    // end interface
    return s.join("\n");
...
function modelClass(api, model, ts) {
    ...
    if (model.extends) {
        ...
        model.eachParentProperty((t, name) => {
            constructorParams.push(`${name}: ${utils_1.schemaObjectTS(t)},`);
