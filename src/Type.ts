...
*/
      ...
      case Kind.REFERENCE:
        if (this.foreignKey) {
          const mdl = this.api.getReference<Model>(this.foreignKey);
          ...
          const mdl = this.api.getReference<Model>(this.referenceModel);
  ...
  derefence(): Type {
    if (this.type == Kind.REFERENCE) {
      return (this.api.getReference(this.referenceModel) as Model).type;
    }
