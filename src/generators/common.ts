...
import { eachModel, eachEnum, schemaObjectTS } from "../utils";

const nemaRoot = path.join(__dirname, "..", "..", "..");
...
export function pretty(dstPath: string) {
  spawnSync(
    path.join(nemaRoot, "node_modules/.bin/prettier.cmd"),
    [
      "--write",
      "--parser",
      "typescript",
...
export function lint(dstPath: string) {
  spawnSync(
    path.join(nemaRoot, "node_modules/.bin/tslint.cmd"),
...
export function copyCommonTemplates(api: OpenAPIObject, dstPath: string) {
...
export function models(api: OpenAPIObject, dstPath: string) {
  eachModel(api, (mdl, modelName) => {
    fs.writeFileSync(path.join(dstPath, "." + mdl.filename), generateModel(api, mdl, mdl.filename));
  });

  eachEnum(api, (mdl, modelName) => {
    fs.writeFileSync(path.join(dstPath, "." + mdl.filename), generateEnum(api, mdl));
  });
}

export function generateModel(api: OpenAPIObject, model: SchemaObject, filename: string): string {
...
export function modelInterface(api: OpenAPIObject, model: SchemaObject, ts: TypescriptFile): string {
  // start interface
  const s = [`export interface ${model.interfaceName} {`];
  _.each(model.properties, (t, name) => {
    s.push(`${name}: ${schemaObjectTS(t)},`);
  });
  s.push(`}`);
  // end interface
...
export function modelClass(api: OpenAPIObject, model: SchemaObject, ts: TypescriptFile): string {
  const s = [];
  ...
  //     constructorParams.push(`${name}: ${schemaObjectTS(t)},`);
...
export function generateEnum(api: OpenAPIObject, model: SchemaObject): string {
